...
// ─── Data Extraction ───
function extractSentMessage(line) {
    if (line.message !== 'API' || !line.raw)
        return null;
...
function reconstructSessions(lines) {
    const sessions = [];
    let current = null;
    let pendingUserMsgs = [];
...
function enrichSessions(sessions, chatEntries) {
...
function computeDiff(fullOutput, sentMessages) {
    ...
    if (!fullOutput && !sent)
        return [];
    if (!fullOutput)
        return [{ type: 'extra', text: sent }];
    if (!sent)
        return [{ type: 'lost', text: fullOutput }];
...
function renderDiff(segments) {
    if (segments.length === 0)
        return '<span class="diff-identical">无法比较</span>';
    const isAllSame = segments.every(s => s.type === 'same');
    if (isAllSame)
        return '<span class="diff-identical">完全一致，无文本丢失</span>';
    let html = '';
...
function generateHtml(sessions, date) {
