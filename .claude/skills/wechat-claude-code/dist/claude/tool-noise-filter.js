...
function isStructuralLine(line) {
    ...
    if (/^\s*[\[\]\{\}]/.test(line))
        return true; // JSON 括号
    if (/\\n/.test(line))
        return true; // JSON 字符串里的换行转义
    if (/^\s*"\w+"\s*:/.test(line))
        return true; // "key": value 行
    if (/^\s*\*\s/.test(line))
        return true; // markdown 斜体列表项
    return false;
}
...
function extractTail(text) {
    ...
    if (lastStructuralIdx < 0 || lastStructuralIdx === lines.length - 1)
        return '';
 ...
 * 任一不满足即原样返回。
 */
export function filterToolNoise(text) {
    if (text.length <= LENGTH_THRESHOLD)
        return text;
    if (!FENCED_JSON.test(text))
        return text;
    if (!URL_OR_PATH.test(text))
        return text;
