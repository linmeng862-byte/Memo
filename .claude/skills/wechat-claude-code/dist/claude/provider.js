...
export function handleStreamLine(line, state, callbacks) {
    if (!line.trim())
        return;
    let obj;
    try {
            ...
            if (evt?.type === 'content_block_start' && evt.content_block?.type === 'tool_use') {
                ...
                try {
                    ...
                    if (parsed.skill) {
                        ...
                        if (callbacks.onText)
                            Promise.resolve(callbacks.onText(msg)).catch(() => { });
                ...
                if (callbacks.onTurnEnd)
                    Promise.resolve(callbacks.onTurnEnd(evt.delta.stop_reason)).catch(() => { });
            }
            break;
        }
        case 'result': {
...
export async function claudeQuery(options) {
    ...
    return new Promise((resolve) => {
        ...
        const parserCallbacks = { onText, onTurnEnd };
