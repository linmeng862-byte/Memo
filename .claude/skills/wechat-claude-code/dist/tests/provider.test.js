...
test('handleStreamLine: content_block_stop 重置 trackingSkill，无回调', () => {
    ...
    let textCalls = 0;
    let turnEndCalls = 0;
    handleStreamLine(JSON.stringify({ type: 'stream_event', event: { type: 'content_block_stop', index: 0 } }), state, { onText: () => textCalls++, onTurnEnd: () => turnEndCalls++ });
...
test('handleStreamLine: message_delta 带 stop_reason 触发 onTurnEnd', () => {
    ...
    }), freshState(), { onTurnEnd: (r) => calls.push(r) });
