...
import { loadPendingQueue, savePendingQueue } from './pending-queue.js';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
function findSafeSplitPoint(text, maxLen) {
    ...
    for (let i = maxLen; i >= maxLen * 0.5; i--) {
        if (sentenceEnd.test(text.slice(i - 1, i)))
            return i;
    }
...
async function handleMessage(msg, account, session, sessionStore, sender, config, sharedCtx, activeControllers, messageQueue) {
    ...
    if (userText.startsWith('/')) {
        ...
        if (result.handled && result.claudePrompt) {
            await sendToClaude(result.claudePrompt, imageItem, fileItem, fromUserId, contextToken, account, session, sessionStore, sender, config, activeControllers);
            return;
        }
    ...
    await sendToClaude(userText, imageItem, fileItem, fromUserId, contextToken, account, session, sessionStore, sender, config, activeControllers);
}
...
async function sendToClaude(userText, imageItem, fileItem, fromUserId, contextToken, account, session, sessionStore, sender, config, activeControllers) {
    ...
    try {
        ...
        function scheduleSend(text, role) {
            if (!text.trim())
                return;
