...
metadata:
  ...
  modified: 2026-07-23T14:35:12.766Z
---
...
## 十七、CoreS3 StackChan · 自建后端 2026-07-23 上线

### WebSocket 地址

`ws://101.42.54.149:9333/xiaozhi/v1/`

### 踩坑记录

1. **端口封锁**：只有 9333 能通。腾讯云或 ISP 封锁了 8000、8003、9334 等非标 HTTP 端口。所有服务必须过 9333。
2. **Docker iptables DROP**：重启 Docker 后 iptables FORWARD 链自动加 DROP，导致外部无法访问容器端口。需要 `iptables -F INPUT` + 重启 Docker + 手工 DNAT。
3. **ufw 防火墙**：`sudo ufw disable`。
4. **NVS 绕过固件编译**：无法重编译固件时，直接写 NVS 分区改变服务器地址。使用 ESP-IDF 的 `nvs_partition_gen.py` 生成 CSV → bin，esptool 烧到 0x9000。关键 key：`websocket/url`、`wifi/ota_url`。
5. **ESP-IDF Docker 编译失败**：国内网络无法拉取 `espressif/idf` 组件，`Failed to resolve component 'json'`。放弃编译，走 NVS 方案。
6. **内存不足**：VPS 2GB，FunASR + Docker 同时跑导致 OOM。需先停 Docker 再编译，或升级 VPS。
7. **人格提示词**：DeepSeek 有台湾腔默认角色，需要在 config.yaml 中用具体例句覆盖。效果仍不完全像 Claude，需 Anthropic API 直连。
8. **WebSocket 客户端库兼容**：Docker 内 `websockets` 库版本不匹配（`create_connection` 参数不同）。桥接脚本改用 HTTP 轮询。

### LLM 接入

- DeepSeek V4：`url: https://api.deepseek.com/v1`，OpenAI 兼容。
- Claude：需 Anthropic API key。`url: https://api.deepseek.com/anthropic` 格式不对，不走 Claude 模型。
- 提示词位置：`~/xiaozhi-server/data/.config.yaml` 第 204 行。
- TTS：EdgeTTS（免费），可换 ElevenLabs。

### MCP 桥（待完成）

CoreS3 已注册 14 个 MCP 工具（head.move、face.expression、camera.take_photo 等）。Zeabur server_lite.py 需加对应工具，通过 HTTP/WS 转发到 VPS 再进容器。

### 运维命令

```bash
cd ~/xiaozhi-server
sudo docker compose down && sudo docker compose up -d   # 重启
sudo docker logs --tail 30 xiaozhi-esp32-server         # 日志
sudo docker exec -it xiaozhi-esp32-server python3 -c "..." # 容器内调试
```

### 固件源码

VPS：`~/stackchan-xiaozhi-firmware/`（Kconfig 已改为 101.42.54.149）
本地：`E:\stackchan-xiaozhi-firmware-main\`
GitHub：`heavenchenggong/stackchan-xiaozhi-firmware`
